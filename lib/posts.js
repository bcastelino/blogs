import fs from 'node:fs';
import path from 'node:path';
import matter from 'gray-matter';
import readingTime from 'reading-time';
import { remark } from 'remark';
import remarkGfm from 'remark-gfm';
import remarkHtml from 'remark-html';
import { SITE_URL } from './site.js';

const postsDir = path.join(process.cwd(), 'content', 'posts');
const writingDir = path.join(process.cwd(), 'content', 'writing');
const basePath = process.env.NEXT_PUBLIC_BASE_PATH || '';

// Root-relative asset URLs (e.g. /posts/pwaf-badge.png) in markdown need the
// production basePath (/blogs) prepended for the GitHub Pages export.
function resolveAssetPaths(html) {
  if (!basePath) return html;
  return html.replace(
    /\b(src|href)="\/(?!\/)/g,
    (_match, attr) => `${attr}="${basePath}/`
  );
}

function slugify(text) {
  return text
    .toLowerCase()
    .replace(/&amp;/g, ' and ')
    .replace(/&#?\w+;/g, '')
    .replace(/[^\w\s-]/g, '')
    .trim()
    .replace(/\s+/g, '-')
    .replace(/-+/g, '-');
}

export function tagSlug(tag) {
  return String(tag)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
}

// Rotating accent palette (the existing 6 design tokens). Each post is
// assigned one deterministically from its slug, so the color is stable
// across reloads and baked into the static HTML (no hydration flash).
const ACCENT_PALETTE = [
  'var(--ios-blue)',
  'var(--ios-orange)',
  'var(--ios-green)',
  'var(--ios-indigo)',
  'var(--ios-pink)',
  'var(--ios-red)',
];

function hashString(str) {
  let hash = 0;
  for (let i = 0; i < str.length; i += 1) {
    hash = (hash << 5) - hash + str.charCodeAt(i);
    hash |= 0;
  }
  return Math.abs(hash);
}

export function getPostAccent(slug) {
  const accent = ACCENT_PALETTE[hashString(slug) % ACCENT_PALETTE.length];
  return {
    accent,
    accentSoft: `color-mix(in oklab, ${accent} 12%, transparent)`,
    accentTint: `color-mix(in oklab, ${accent} 8%, var(--color-fill-1))`,
    accentBorder: `color-mix(in oklab, ${accent} 32%, var(--color-border))`,
  };
}

export function getPostSlugs() {
  if (!fs.existsSync(postsDir)) return [];
  return fs
    .readdirSync(postsDir)
    .filter((file) => file.endsWith('.md'))
    .map((file) => file.replace(/\.md$/, ''));
}

function readPost(slug) {
  const fullPath = path.join(postsDir, `${slug}.md`);
  const raw = fs.readFileSync(fullPath, 'utf8');
  return matter(raw);
}

const DEFAULT_AUTHOR = 'Brian Castelino';
const DEFAULT_AUTHOR_URL = 'https://www.linkedin.com/in/cas7elino/';

export function getPostMeta(slug) {
  const { data, content } = readPost(slug);
  return {
    slug,
    title: data.title ?? slug,
    seoTitle: data.seoTitle ?? '',
    metaDescription: data.metaDescription ?? '',
    date: data.date ? new Date(data.date).toISOString() : null,
    excerpt: data.excerpt ?? '',
    tags: Array.isArray(data.tags) ? data.tags : [],
    author: data.author ?? DEFAULT_AUTHOR,
    authorUrl: data.authorUrl ?? DEFAULT_AUTHOR_URL,
    authorSlug: data.authorSlug ?? tagSlug(data.author ?? DEFAULT_AUTHOR),
    updated: data.updated ? new Date(data.updated).toISOString() : null,
    featured: data.featured === true,
    hidden: data.hidden === true,
    noindex: data.noindex === true,
    faq: Array.isArray(data.faq)
      ? data.faq.filter((item) => item && item.question && item.answer)
      : [],
    takeaways: Array.isArray(data.takeaways)
      ? data.takeaways.filter((item) => typeof item === 'string' && item.trim())
      : [],
    readingTime: readingTime(content).text,
    readingMinutes: Math.max(1, Math.round(readingTime(content).minutes)),
  };
}

export function getAllPosts() {
  return getPostSlugs()
    .map(getPostMeta)
    .sort((a, b) => {
      if (!a.date) return 1;
      if (!b.date) return -1;
      return new Date(b.date) - new Date(a.date);
    });
}

// Posts shown on the front page: excludes any post flagged `hidden`.
// Hidden posts stay published/indexed and still appear in the archive,
// feeds, and sitemap; they are only kept off the home shelf.
export function getVisiblePosts() {
  return getAllPosts().filter((post) => !post.hidden);
}

// The cover story: the first post flagged `featured`, falling back to the
// newest visible post if none is flagged.
export function getFeaturedPost() {
  const visible = getVisiblePosts();
  return visible.find((post) => post.featured) ?? visible[0] ?? null;
}

export function getAllTags() {
  const map = new Map();
  for (const post of getAllPosts()) {
    for (const tag of post.tags) {
      const slug = tagSlug(tag);
      if (!slug) continue;
      if (!map.has(slug)) {
        map.set(slug, { slug, name: tag, count: 0, posts: [] });
      }
      const entry = map.get(slug);
      entry.count += 1;
      entry.posts.push(post);
    }
  }
  return [...map.values()].sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
}

export function getTag(slug) {
  return getAllTags().find((tag) => tag.slug === slug) ?? null;
}

function compactTableLinks(inner) {
  return inner.replace(/<a href="([^"]+)">([^<]*)<\/a>/g, (match, href, text) => {
    const decodedText = text.replace(/&#x26;/g, '&').replace(/&amp;/g, '&').trim();
    if (!/^https?:\/\//i.test(decodedText)) return match;
    const label = href
      .replace(/&#x26;/g, '&')
      .replace(/&amp;/g, '&')
      .replace(/^https?:\/\//i, '')
      .replace(/^www\./i, '')
      .split(/[/?#]/)[0];
    return `<a href="${href}" target="_blank" rel="noopener noreferrer" class="table-link">${label}</a>`;
  });
}

// Wrap each table in a scroll container and tag it with its column count so
// CSS can pick shrink-to-fit (narrow) vs horizontal-scroll (wide) behaviour.
function enhanceTables(html) {
  return html.replace(/<table>([\s\S]*?)<\/table>/g, (_match, inner) => {
    const firstRow = inner.match(/<tr>([\s\S]*?)<\/tr>/);
    let cols = 0;
    if (firstRow) {
      const ths = firstRow[1].match(/<th[\s>]/g);
      const tds = firstRow[1].match(/<td[\s>]/g);
      cols = (ths ? ths.length : 0) || (tds ? tds.length : 0);
    }
    const compacted = compactTableLinks(inner);
    const wide = cols >= 4 ? ' is-wide' : '';
    return `<div class="table-wrap${wide}" data-cols="${cols}"><div class="table-scroll"><table>${compacted}</table></div></div>`;
  });
}

async function processMarkdownContent(content) {
  const processed = await remark()
    .use(remarkGfm)
    .use(remarkHtml, { sanitize: false })
    .process(content);

  let html = processed.toString();
  const headings = [];
  const seen = {};
  let h2Count = 0;

  // Inject ids into h2/h3 headings and collect a table of contents.
  // h2s also get a numbered accent span matching the reading-rail numbering.
  html = html.replace(/<h([23])>([\s\S]*?)<\/h\1>/g, (_match, level, inner) => {
    const text = inner
      .replace(/<[^>]+>/g, '')
      .replace(/&amp;/g, '&')
      .replace(/&#?\w+;/g, '')
      .trim();
    let id = slugify(text) || 'section';
    if (seen[id] != null) {
      seen[id] += 1;
      id = `${id}-${seen[id]}`;
    } else {
      seen[id] = 0;
    }
    headings.push({ id, text, level: Number(level) });
    if (level === '2') {
      h2Count += 1;
      const num = String(h2Count).padStart(2, '0');
      return `<h2 id="${id}"><span class="sec-num" aria-hidden="true">${num}</span>${inner}</h2>`;
    }
    return `<h${level} id="${id}">${inner}</h${level}>`;
  });

  html = enhanceTables(html);
  html = resolveAssetPaths(html);

  return { html, headings };
}

async function processInlineMarkdown(text) {
  const processed = await remark()
    .use(remarkGfm)
    .use(remarkHtml, { sanitize: false })
    .process(text);
  return processed
    .toString()
    .trim()
    .replace(/^<p>/, '')
    .replace(/<\/p>$/, '')
    .trim();
}

async function processMarkdown(raw) {
  const { content } = matter(raw);
  return processMarkdownContent(content);
}

export async function getPost(slug) {
  const meta = getPostMeta(slug);
  const { html, headings } = await processMarkdownContent(readPost(slug).content);
  const takeaways = await Promise.all(meta.takeaways.map(processInlineMarkdown));
  return {
    ...meta,
    takeaways,
    html,
    headings,
  };
}

// Produce the public, LLM-friendly representation used by the reading rail
// and the per-post Markdown route. Keep authoring-only frontmatter out of the
// response while retaining the metadata a reader needs to understand and cite
// the article.
export function getPostMarkdown(slug) {
  const { content } = readPost(slug);
  const meta = getPostMeta(slug);
  const canonical = `${SITE_URL}/blog/${slug}/`;
  const metadata = [
    `- Author: ${meta.authorUrl ? `[${meta.author}](${meta.authorUrl})` : meta.author}`,
    meta.date ? `- Published: ${meta.date.slice(0, 10)}` : null,
    meta.updated && meta.updated !== meta.date
      ? `- Updated: ${meta.updated.slice(0, 10)}`
      : null,
    meta.tags.length > 0 ? `- Topics: ${meta.tags.join(', ')}` : null,
    `- Reading time: ${meta.readingTime}`,
    `- Canonical URL: ${canonical}`,
  ].filter(Boolean);

  return [
    `# ${meta.title}`,
    '',
    meta.excerpt ? `> ${meta.excerpt}` : null,
    meta.excerpt ? '' : null,
    ...metadata,
    '',
    '---',
    '',
    content.trim(),
    '',
  ]
    .filter((line) => line !== null)
    .join('\n');
}

export async function getDocsPage(slug) {
  const fullPath = path.join(writingDir, `${slug}.md`);
  const raw = fs.readFileSync(fullPath, 'utf8');
  const { data } = matter(raw);
  const { html, headings } = await processMarkdown(raw);
  return {
    title: data.title ?? slug,
    html,
    headings,
  };
}

export async function getDocsPageSections(slug) {
  const fullPath = path.join(writingDir, `${slug}.md`);
  const raw = fs.readFileSync(fullPath, 'utf8');
  const { content } = matter(raw);

  const sections = [];
  let current = null;

  for (const line of content.trimStart().split('\n')) {
    if (line.startsWith('## ')) {
      if (current) sections.push(current);
      current = { heading: line, lines: [] };
    } else {
      if (!current) {
        current = { heading: null, lines: [] };
      }
      current.lines.push(line);
    }
  }
  if (current) sections.push(current);

  const seen = {};

  return Promise.all(
    sections
      .filter((section) => section.heading || section.lines.join('').trim().length > 0)
      .map(async (section) => {
        const heading = section.heading ? section.heading.replace(/^## /, '').trim() : '';
        const title = heading || 'Introduction';
        const bodyMarkdown = section.lines.join('\n').trim();
        const fullMarkdown = section.heading
          ? `${section.heading}\n\n${bodyMarkdown}`
          : bodyMarkdown;
        const { html } = await processMarkdownContent(bodyMarkdown);

        let id = slugify(title) || 'section';
        if (seen[id] != null) {
          seen[id] += 1;
          id = `${id}-${seen[id]}`;
        } else {
          seen[id] = 0;
        }

        return {
          id,
          title,
          markdown: fullMarkdown,
          html,
          hasHeading: !!section.heading,
        };
      })
  );
}

export function formatDate(iso) {
  if (!iso) return '';
  return new Date(iso).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
}
